;(function (root) {
    ...
    CrossStorageHub.init = function (permissions, storageType) {
        var available = true;
        CrossStorageHub.storage = storageType === 'localStorage' ? window.localStorage : window.sessionStorage;
        // Return if localStorage is unavailable, or third party
        // access is disabled
        try {
            if (!CrossStorageHub.storage) available = false;
        } catch (e) {
            available = false;
        }
    ...
    CrossStorageHub._set = function (params) {
        CrossStorageHub.storage.setItem(params.key, params.value);
    };
    ...
    CrossStorageHub._get = function (params) {
        ...
        storage = CrossStorageHub.storage;
        result = [];
    ...
    CrossStorageHub._del = function (params) {
        for (var i = 0; i < params.keys.length; i++) {
            CrossStorageHub.storage.removeItem(params.keys[i]);
        }
    };
    ...
    CrossStorageHub._getKeys = function (params) {
        ...
        keys = [];
        length = CrossStorageHub.storage.length;

        for (i = 0; i < length; i++) {
            keys.push(CrossStorageHub.storage.key(i));
        }
